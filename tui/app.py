...
def _read_key(timeout: float = _KEY_READ_TIMEOUT_SECONDS) -> str:
    return read_key(sys.stdin, timeout=timeout, escape_timeout=_ESCAPE_SEQUENCE_TIMEOUT_SECONDS)
...
def _handle_key(key: str, state: AppState, config: Config, db: Database | None = None) -> None:
    if key == "QUIT":
        ...
        if state.current_screen in _SCREENS_THAT_RESCAN_ON_REFRESH:
            hosts, message = trigger_scan(config)
...
def _plain_command_to_key(command: str) -> str:
    ...
    aliases = {
        ...
        "g": "HOME", "G": "END", "home": "HOME", "end": "END", "enter": "ENTER",
        "open": "ENTER", "r": "REFRESH", "refresh": "REFRESH", "s": "SYNC", "sync": "SYNC",
...
def _run_live_tui(config: Config, db: Database, console: Console, mode: TerminalMode) -> None:
    state = AppState()
...
def run_tui(config: Config, db: Database, mode: str | TerminalMode = TerminalMode.AUTO) -> None:
    capabilities = detect_terminal_capabilities(sys.stdin, sys.stdout, requested_mode=mode)
    ...
    if capabilities.reason:
        log.info(
            ...
            capabilities.term or "<unset>", capabilities.mode.value, capabilities.reason,
        )
    try:
