...
class DiscoveryPage(BasePage):
    def __init__(self, config: Config, database: Database, state: ApplicationState, tasks: BackgroundTaskRunner) -> None:
        ...
        self.mode=Gtk.ComboBoxText(); [self.mode.append(v,v.capitalize()) for v in ("passive","active","hybrid")]; self.mode.set_active_id(config.discovery.mode); row.append(self.mode); options.append(row)
        ...
        row=Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL,spacing=12); label=Gtk.Label(label="Command timeout (seconds)",xalign=0); label.set_hexpand(True); row.append(label)
        ...
        expander=Gtk.Expander(label="Scan options"); expander.set_child(options); expander.set_expanded(True); self.append(expander)
        ...
        self.status=Gtk.Label(label="Ready",xalign=0,wrap=True); self.append(self.status)
    ...
    def _scan_finished(self,snapshot:DiscoverySnapshot)->None:
        ...
        self.state.discovery.status_message=(f"Scan completed: {snapshot.scanned} host(s) observed. New {snapshot.new_devices}, updated {snapshot.updated_devices}, offline {snapshot.offline_devices}." if snapshot.error is None else snapshot.error)
