...
class BasePage(Gtk.Box):
    def __init__(self, title: str, subtitle: str = "") -> None:
        super().__init__(orientation=Gtk.Orientation.VERTICAL, spacing=18); self.set_margin_top(28); self.set_margin_bottom(28); self.set_margin_start(32); self.set_margin_end(32)
        heading=Gtk.Label(label=title,xalign=0); heading.add_css_class("title-1"); self.append(heading)
        if subtitle: label=Gtk.Label(label=subtitle,xalign=0,wrap=True); label.add_css_class("dim-label"); self.append(label)

def _section(title:str,child:Gtk.Widget)->Gtk.Box:
    box=Gtk.Box(orientation=Gtk.Orientation.VERTICAL,spacing=8); heading=Gtk.Label(label=title,xalign=0); heading.add_css_class("heading"); box.append(heading); box.append(child); return box

class DashboardPage(BasePage):
    def __init__(self,config:Config,database:Database,state:ApplicationState)->None:
        super().__init__("Dashboard","NetFather Linux network management"); self.database=database; self.state=state; self.status=Gtk.Label(xalign=0,wrap=True); self.status.add_css_class("card"); self.append(self.status); self.refresh(); button=Gtk.Button(label="Refresh"); button.set_halign(Gtk.Align.START); button.connect("clicked",lambda _b:self.refresh()); self.append(button)
    def refresh(self)->None:
        network=get_network_status()
        with self.database.session() as session: count=session.scalar(select(func.count(Device.id))) or 0; online=session.scalar(select(func.count(Device.id)).where(Device.online.is_(True))) or 0
        self.state.network.interface=network.interface; self.state.network.local_ip=network.local_ip; self.state.network.gateway=network.gateway; self.state.network.known_devices=count; self.state.network.online_devices=online
